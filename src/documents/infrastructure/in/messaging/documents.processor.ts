 ...
 * @description Worker de mensajes para BullMQ. Consume los jobs de la cola
 * "blockchain-queue", coordina el registro real en blockchain mediante el
 * puerto de salida, y envía notificaciones a través del puerto INotificationPort.
 *
 * Estrategia de notificación según el modo del job:
 * - NORMAL   (deferred=false): notificación en tiempo real por WebSocket.
 * - DIFERIDO (deferred=true) : notificación asíncrona por el canal diferido.
 *
 * Gestión de errores:
...
import * as blockchainPort_1 from '../../../application/ports/out/blockchain.port';
import * as notificationPort from '../../../application/ports/out/notification.port';
...
export class DocumentsProcessor extends WorkerHost {
  constructor(
    @Inject(blockchainPort_1.I_BLOCKCHAIN_PORT)
    private readonly blockchainPort: blockchainPort_1.IBlockchainPort,
    @Inject(notificationPort.I_NOTIFICATION_PORT)
    private readonly notificationPort: notificationPort.INotificationPort,
  ) {
    super();
  }
   ...
   * completamente en INotificationPort la elección del canal.
   *
  ...
  private async handleRegisterDocument(
    ...
    try {
      const txHash = await this.blockchainPort.registrarHash(hash);
