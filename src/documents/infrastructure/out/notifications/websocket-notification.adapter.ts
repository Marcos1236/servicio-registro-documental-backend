/**
 * @file websocket-notification.adapter.ts
 ...
 *
 * Al utilizar @WebSocketGateway, NestJS inyecta automáticamente
...
export class WebSocketNotificationAdapter implements NotificationStrategy {
  @WebSocketServer()
  server!: Server;
  ...
  send(payload: NotificationPayload): Promise<void> {
    ...
    if (!jobId || !status) {
      console.warn(
        `[WebSocketNotificationAdapter] Payload incompleto: falta jobId o status.`,
      );
    ...
    if (status === 'completed' && txHash) {
      this.server
        .to(`job-${jobId}`)
        .emit('job-status', { status: 'completed', txHash });
    } else if (status === 'failed' && error) {
      this.server
        .to(`job-${jobId}`)
        .emit('job-status', { status: 'failed', error });
    }
