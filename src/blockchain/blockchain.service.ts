...
export class BlockchainService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    this.contractAddress = this.configService.get<string>('REGISTRO_CONTRATO_ADDRESS')!;

    if (!rpcUrl || !this.contractAddress) {
      throw new Error('[BlockchainService] Faltan variables RPC o Contract Address en el .env');
    }
  ...
  async registrarHash(hash: string): Promise<string> {
    console.log('[BlockchainService] Pidiendo autorización al gestor de secretos...');
    const privateKey = await this.secretManager.getPrivateKey();

    const tempWallet = new ethers.Wallet(privateKey, this.provider);
    const writableContract = new ethers.Contract(this.contractAddress, this.abi, tempWallet);

    console.log('[BlockchainService] Firmando transacción y enviando al nodo...');
  ...
  async verificarHash(hash: string) {
    const result = await this.readOnlyContract.verificar(hash);
    return {
      existe: result[0],
      emisor: result[1],
